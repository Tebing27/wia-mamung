...
import { Search, X, ChevronRight, Filter, MapPin, Phone, Globe, Instagram, Facebook } from "lucide-react";
...
export default function UmkmCatalog({ onUmkmClick, onBack, onNavigateToSection }: UmkmCatalogProps) {
            ...
            <div className="relative md:w-48">
              ...
              >
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-sm md:text-base">{selectedCategory}</span>
                </div>
                <ChevronRight className={`w-5 h-5 transition-transform ${showCategoryDropdown ? 'rotate-90' : ''}`} />
              </button>
                  ...
                  {categories.map((cat) => (
                    <button
                      ...
                      className={`w-full px-4 py-3 text-left text-sm hover:bg-gray-50 flex items-center justify-between transition-colors ${
                        selectedCategory === cat.value ? 'bg-[#FFC107] font-semibold text-black' : ''
                      }`}
                    >
                      <span>{cat.label}</span>
                      {selectedCategory === cat.value && <span className="text-lg">✓</span>}
                    </button>
                  ))}
                </div>
              )}
      ...
      <div className="container mx-auto px-4 md:px-20 pb-12">
        {filteredUmkm.length > 0 ? (
                  ...
                  <div className="space-y-2 mb-4 text-xs text-gray-600">
                    {umkm.address && (
                      <div className="flex gap-2 items-start">
                        <MapPin className="w-4 h-4 mt-0.5 flex-shrink-0 text-[#0B4EA2]" />
                        <p className="line-clamp-2">{umkm.address}</p>
                      </div>
                    )}
                    {umkm.phone && (
                      <div className="flex gap-2 items-center">
                        <Phone className="w-4 h-4 flex-shrink-0 text-[#0B4EA2]" />
                        <a href={`tel:${umkm.phone}`} className="hover:text-[#0B4EA2] transition-colors">
                          {umkm.phone}
                        </a>
                      </div>
                    )}
                  ...
                  <div className="flex gap-2">
                    ...
                    {onUmkmClick && (
                      <button
                        ...
                        className="flex-1 px-4 py-2 bg-[#FFC107] text-black text-sm font-medium rounded-lg hover:bg-[#FFB800] transition-colors"
                      >
                        Detail
                      </button>
                    )}
          ...
          <div className="text-center py-12">
            <p className="text-gray-500 text-lg mb-4">Tidak ada UMKM yang ditemukan</p>
            <button
              onClick={() => {
                setSearchQuery("");
