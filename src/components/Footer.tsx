// src/components/Footer.tsx (DIPERBARUI)

import { Link, useLocation, useNavigate } from "react-router-dom"; // <-- 1. Impor

export default function Footer() {
  const navigate = useNavigate(); // <-- 2. Gunakan hook
  const location = useLocation(); // <-- 2. Gunakan hook

  // 3. Logika klik BARU
  const handleClick = (e: React.MouseEvent<HTMLAnchorElement>, to: string) => {
    e.preventDefault();

    // Asumsi link footer selalu ke hash di halaman utama
    const hash = to; // cth: "#beranda"

    if (location.pathname === "/") {
      // Jika sudah di home, cukup scroll
      if (hash === "#beranda") {
      ...
      // Jika di halaman lain, navigasi ke home + hash
      // (HomePage.tsx akan menangani scroll)
      navigate(`/${hash}`);
    }
  };
      ...
      <div className="container mx-auto px-6 md:px-12 lg:px-20">
        ...
        <div className="mb-8 md:mb-12">
          ...
          <p className="text-sm text-white/80 mt-2">
            Platform untuk menemukan dan mendukung UMKM lokal
          </p>
        </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 gap-8 md:gap-12 mb-8 md:mb-12">
            ...
            <ul className="space-y-3 text-sm md:text-base">
              <li>
                ...
                <Link
                  ...
                  className="hover:underline hover:text-[#FFC107] transition-colors"
                >
                  Beranda
                </Link>
              </li>
              <li>
                <Link
                  to="#kategori"
                  onClick={(e) => handleClick(e, "#kategori")}
                  className="hover:underline hover:text-[#FFC107] transition-colors"
                >
                  Kategori
                </Link>
          ...
          {/* ... sisa link (Tentang, Ikuti Kami) biarkan saja karena href="#" atau eksternal ... */}
          {/* About Links */}
          <div>
          ...
          <div>
            ...
            <div className="flex gap-4">
              {/* Link eksternal biarkan pakai <a> */}
              <a
        ...
        <div className="flex flex-col md:flex-row justify-between items-center gap-4 pt-8 border-t border-white/20">
          ...
          <div className="flex flex-col md:flex-row gap-4 md:gap-8 text-center">
            <a
              href="#"
              className="text-xs md:text-sm hover:underline hover:text-[#FFC107] transition-colors"
            >
              Syarat & Ketentuan
            </a>
            <a
