import { ChevronLeft, ChevronRight, ChevronUp, ChevronDown, Search, Filter, X, MapPin, Phone, Globe, Instagram, Facebook } from "lucide-react";
...
export default function LocationSection({ selectedCategory: propSelectedCategory, selectedUmkmId: propSelectedUmkmId }: LocationSectionProps) {
        ...
        <section id="lokasi" className="py-12 md:py-20 px-4 md:px-20 bg-white">
            <div className="container mx-auto">
                    ...
                    <div className="bg-[#003F88] rounded-t-lg p-3">
                        ...
                        <div className="flex gap-2 mb-3 relative">
                            ...
                            <div className="flex-1 relative">
                                ...
                                >
                                    ...
                                    <span className="flex-1 text-left truncate">{activeCategory || "Semua"}</span>
                                    ...
                                    <div className="absolute top-full left-0 right-0 mt-1 bg-white rounded-lg shadow-lg z-10 max-h-48 overflow-y-auto">
                                        {categories.map((cat) => (
                                            <button
                                                ...
                                                className={`w-full px-3 py-2 text-xs text-left hover:bg-gray-50 flex items-center justify-between ${activeCategory === cat.value ? 'bg-[#FFC107] font-semibold' : ''
                                                    }`}
                                            >
                                                <span>{cat.label}</span>
                        ...
                        {filteredUmkmList.length === 0 ? (
                            <div className="bg-white rounded-lg p-6 mb-3 text-center">
                                ...
                                <p className="text-sm font-semibold mb-1 text-gray-700">Tidak ada hasil</p>
                                    ...
                                    <div className="flex gap-3">
                                        ...
                                        <div className="flex-1">
                                            <div className="flex items-start justify-between mb-1">
                                                ...
                                                <span className="bg-[#FFC107] text-black text-[9px] font-bold px-2 py-0.5 rounded flex-shrink-0 ml-1">
                                    ...
                                    <div className="absolute top-full left-0 right-0 mt-1 bg-white rounded-lg shadow-lg z-10 max-h-60 overflow-y-auto">
                                        {categories.map((cat) => (
                                            <button
                                                key={cat.value}
                                                onClick={() => handleCategorySelect(cat.value)}
                                                className={`w-full px-3 py-2 text-sm text-left hover:bg-gray-50 flex items-center justify-between ${activeCategory === cat.value ? 'bg-[#FFC107] font-semibold' : ''
                                                    }`}
                                            >
                                                <span>{cat.label}</span>
                                                {activeCategory === cat.value && (
                        ...
                        <div className="flex-1 flex flex-col gap-3 min-h-0">
                            {filteredUmkmList.length === 0 ? (
                                ...
                                visibleUmkmList.map((umkm) => (
                                    <div
                                        key={umkm.id}
                                        ref={(el) => { cardRefs.current[umkm.id] = el; }}
                                        className="bg-white rounded-xl p-3 shadow-sm flex-shrink-0 h-[135px] transition-all duration-300"
                                    >
                ...
                {selectedUmkmId && (() => {
                    ...
                    return (
                        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4" onClick={() => setSelectedUmkmId(null)}>
                            <div className="bg-white rounded-2xl shadow-2xl w-full max-w-[420px] max-h-[90vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
                                {/* Image Carousel */}
                                ...
                                <div className="p-5">
                                    ...
                                    <div className="flex items-start justify-between mb-3">
                                        <h3 className="text-2xl font-bold text-gray-900">{selectedUmkm.name}</h3>
                                        <span className="bg-[#FFC107] text-black text-sm font-bold px-3 py-1 rounded-md flex-shrink-0 ml-2">
                                    ...
                                    <div className="flex items-start gap-2 mb-4 text-gray-600">
                                        ...
                                        <p className="text-sm leading-relaxed">{selectedUmkm.address}</p>
                                    </div>
                                    ...
                                    <div className="grid grid-cols-2 gap-2 mb-4">
                                        <a href={`https://instagram.com/${selectedUmkm.instagram}`} target="_blank" rel="noopener noreferrer" className="flex items-center gap-2 text-sm text-gray-700 hover:text-pink-600">
                                            <Instagram className="w-5 h-5 text-pink-600" />
                                            <span>{selectedUmkm.instagram}</span>
                                        </a>
                                        <a href={`https://${selectedUmkm.website}`} target="_blank" rel="noopener noreferrer" className="flex items-center gap-2 text-sm text-gray-700 hover:text-blue-600">
                                            <Globe className="w-5 h-5 text-blue-600" />
                                            <span>{selectedUmkm.website}</span>
                                        </a>
                                        <a href={`https://facebook.com/${selectedUmkm.facebook}`} target="_blank" rel="noopener noreferrer" className="flex items-center gap-2 text-sm text-gray-700 hover:text-blue-700">
                                            <Facebook className="w-5 h-5 text-blue-700" />
                                            <span>{selectedUmkm.facebook}</span>
                                        </a>
                                        <a href={`https://wa.me/${selectedUmkm.phone}`} target="_blank" rel="noopener noreferrer" className="flex items-center gap-2 text-sm text-gray-700 hover:text-green-600">
                                    ...
                                    <div className="flex gap-3">
                                        {/* Google Maps Button */}
                                        <a
                                            href={selectedUmkm.mapsUrl || `https://www.google.com/maps/search/?api=1&query=${selectedUmkm.latitude},${selectedUmkm.longitude}`}
                                            target="_blank"
