...
export default function CategorySection({
  ...
  const isAtEndDesktop =
    currentIndexDesktop >= (totalPagesDesktop - 1) * itemsPerPageDesktop;

  const isAtStartMobile = currentIndexMobile === 0;
  const isAtEndMobile =
    currentIndexMobile >= (totalPagesMobile - 1) * itemsPerPageMobile;

  const handleViewLocation = (categoryName: string) => {
    onCategoryClick(categoryName);
  };

  const visibleCategoriesDesktop = categories.slice(
    currentIndexDesktop,
    currentIndexDesktop + itemsPerPageDesktop
  );
  const visibleCategoriesMobile = categories.slice(
    currentIndexMobile,
    currentIndexMobile + itemsPerPageMobile
  );
  return (
      ...
      <div className="container mx-auto max-w-7xl">
          ...
          <div className="grid grid-cols-2 gap-4">
            {visibleCategoriesMobile.map((category, index) => (
              <div
                ...
                className={`${
                  category.bgColor
                } rounded-2xl p-4 flex flex-col card-hover animate-scale-in animate-delay-${
                  index * 100
                }`}
              >
        ...
        <div className="hidden md:grid lg:hidden grid-cols-12 gap-8 items-center">
          ...
          <div className="col-span-3">
            ...
            {/* Navigasi: MENGGUNAKAN LOGIKA & STATE MOBILE (2 items) */}
          ...
          {/* Kanan: Kartu Kategori (MENGGUNAKAN 2 KARTU) */}
          <div className="col-span-9">
            {/* Menggunakan grid-cols-2 */}
            <div className="grid grid-cols-2 gap-6">
              {/* Menggunakan visibleCategoriesMobile */}
              {visibleCategoriesMobile.map((category, index) => (
                <div
                  key={category.id}
                  className={`${
                    category.bgColor
                  } rounded-2xl p-6 flex flex-col card-hover animate-scale-in animate-delay-${
                    index * 100
                  }`}
                >
